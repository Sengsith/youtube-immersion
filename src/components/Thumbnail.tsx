  ...
  return (
    <div className="trending-video-thumbnail flex flex-col gap-4 mb-8">
      <Link className="relative" to={`/watch?v=${video.id}`} state={{ video }}>
        ...
        <p className="video-duration text-sm absolute bottom-2 right-2 bg-black bg-opacity-70 px-2 py-1 rounded-md">{video.duration}</p>
      </Link>
      <div className="video-details flex flex-col gap-1 px-4">
        ...
        <div className="thumbnail-channel flex gap-2 items-center">
          <img className="rounded-full w-10 self-center" src={video.channelThumbnail.url} alt={video.channelTitle} />
